module test_pic_lapack_interfaces_syev
   use testdrive, only: new_unittest, unittest_type, error_type, check
   use pic_lapack_interfaces, only: pic_syev
   use pic_types, only: sp, dp, default_int
   implicit none
   private
   public :: collect_pic_lapack_syev_tests

contains

   subroutine collect_pic_lapack_syev_tests(testsuite)
      type(unittest_type), allocatable, intent(out) :: testsuite(:)

      testsuite = [ &
                  new_unittest("test_ssyev_basic", test_ssyev_basic), &
                  new_unittest("test_dsyev_basic", test_dsyev_basic), &
                  new_unittest("test_dsyev_eigenvalues_only", test_dsyev_eigenvalues_only), &
                  new_unittest("test_dsyev_diagonal", test_dsyev_diagonal), &
                  new_unittest("test_dsyev_identity", test_dsyev_identity) &
                  ]

   end subroutine collect_pic_lapack_syev_tests

   subroutine test_ssyev_basic(error)
      ...
      call pic_syev(A, W, info=info)

      call check(error, info == 0)
      if (allocated(error)) return

      ! Eigenvalues should be sorted ascending
      ! For this tridiagonal Toeplitz matrix: eigenvalues are 2 - sqrt(2), 2, 2 + sqrt(2)
      ...
      if (allocated(error)) return
   end subroutine test_ssyev_basic

   subroutine test_dsyev_basic(error)
      type(error_type), allocatable, intent(out) :: error
      real(dp) :: A(3, 3), W(3)
      ...
      if (allocated(error)) return
   end subroutine test_dsyev_basic

   subroutine test_dsyev_eigenvalues_only(error)
      type(error_type), allocatable, intent(out) :: error
      real(dp) :: A(2, 2), W(2)
      real(dp), parameter :: tol = 1.0e-10_dp
      integer(default_int) :: info
      ...
      call pic_syev(A, W, jobz='N', info=info)
      ...
      if (allocated(error)) return
   end subroutine test_dsyev_eigenvalues_only

   subroutine test_dsyev_diagonal(error)
      type(error_type), allocatable, intent(out) :: error
      real(dp) :: A(3, 3), W(3)
      real(dp), parameter :: tol = 1.0e-10_dp
      integer(default_int) :: info
      ...
      if (allocated(error)) return
   end subroutine test_dsyev_diagonal

   subroutine test_dsyev_identity(error)
      type(error_type), allocatable, intent(out) :: error
      real(dp) :: A(4, 4), W(4)
      real(dp), parameter :: tol = 1.0e-10_dp
   ...
   end subroutine test_dsyev_identity

end module test_pic_lapack_interfaces_syev
